...
class DetailsController extends FrontController
{
	...
	public function index($postId)
	{
		...
		// Get Custom Fields
		$customFields = $this->getPostFieldsValues(data_get($post, 'category.id'), data_get($post, 'id'));
		...
		if (config('plugins.reviews.installed')) {
			try {
				$rvPost = \extras\plugins\reviews\app\Models\Post::query()
					->withoutGlobalScopes([VerifiedScope::class, ReviewedScope::class])
					->find(data_get($post, 'id'));
