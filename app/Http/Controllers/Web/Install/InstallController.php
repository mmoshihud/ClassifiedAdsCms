	...
	public function database()
	{
		...
		if (request()->isMethod('POST')) {
			...
			try {
				...
				$dsn = (request()->filled('socket'))
					? $driver . ':unix_socket=' . request()->input('socket') . ';dbname=' . request()->input('database') . ';charset=utf8'
					: $driver . ':host=' . request()->input('host') . ';port=' . $port . ';dbname=' . request()->input('database') . ';charset=utf8';
