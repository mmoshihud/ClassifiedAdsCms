...
class SecuritySetting
{
	public static function getValues($value, $disk)
	{
		if (empty($value)) {
			...
			if (!isset($value['password_min_length'])) {
				$value['password_min_length'] = '6';
			}
			if (!isset($value['password_max_length'])) {
				$value['password_max_length'] = '60';
			}
			if (!isset($value['password_letters_required'])) {
				$value['password_letters_required'] = '0';
			}
			if (!isset($value['password_mixedCase_required'])) {
				$value['password_mixedCase_required'] = '0';
			}
			if (!isset($value['password_numbers_required'])) {
				$value['password_numbers_required'] = '0';
			}
			if (!isset($value['password_symbols_required'])) {
				$value['password_symbols_required'] = '0';
			}
			if (!isset($value['password_uncompromised_required'])) {
				$value['password_uncompromised_required'] = '0';
			}
			if (!isset($value['password_uncompromised_threshold'])) {
				$value['password_uncompromised_threshold'] = '0';
			}
			
			if (!isset($value['email_validator_rfc'])) {
				$value['email_validator_rfc'] = '1';
			}
			if (!isset($value['email_validator_strict'])) {
				$value['email_validator_strict'] = '0';
			}
			if (!isset($value['email_validator_dns'])) {
				$value['email_validator_dns'] = '0';
			}
			if (!isset($value['email_validator_spoof'])) {
				$value['email_validator_spoof'] = '0';
			}
			if (!isset($value['email_validator_filter'])) {
				$value['email_validator_filter'] = '0';
			}
