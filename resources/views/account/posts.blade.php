							...
							<form name="listForm" method="POST" action="{{ url('account/posts/' . $pagePath . '/delete') }}">
								...
								>
									...
									foreach($posts as $key => $post):
										...
										$postImg = imgUrl(data_get($post, 'pictures.0.filename', $defaultPicture), 'medium');
									...
									<tr>
										...
										<td style="width:10%" class="action-td">
											<div>
												...
												@endif
												@if ($pagePath == 'list' && isVerifiedPost($post) && data_get($post, 'archived') == 0)
													<p>
												...
												@endif
												@if ($pagePath == 'archived' && data_get($post, 'user_id') == $user->id && data_get($post, 'archived') == 1)
													<p>
